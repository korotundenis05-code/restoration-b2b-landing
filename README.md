...
- Для сборки единого коллажа «до / после» используйте
  `scripts/build_before_after_collage.py --before SOURCE --after SOURCE --output OUTPUT`.
  Скрипту нужен Pillow; для исходников HEIC также нужен `heif-convert` из libheif.
  Если программа установлена не в `PATH`, передайте её путь через переменную
  `HEIF_CONVERT`. Для набора работ 36-42 выбранные пары, контрольные суммы и
  имена выходных файлов зафиксированы в
  `scripts/case-gallery-36-42-manifest.json`; перед пересборкой задайте корень
  исходников через `CASE_GALLERY_SOURCE_ROOT`.
