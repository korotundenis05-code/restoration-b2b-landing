...
class BingSeoSignalTests(unittest.TestCase):
    ...
    def test_homepage_modified_dates_match_release(self):
        ...
        self.assertEqual(next(item for item in graph if has_type(item, "WebSite"))["dateModified"], "2026-09-24")
        self.assertEqual(next(item for item in graph if has_type(item, "WebPage"))["dateModified"], "2026-09-24")
        sitemap = (ROOT / "sitemap.xml").read_text()
        homepage = next(block for block in sitemap.split("<url>")[1:] if "<loc>https://restb2b.fun/</loc>" in block)
        self.assertIn("<lastmod>2026-09-24</lastmod>", homepage)
